...
const extractProductData = async (url, browser) => {
  try {
    ...
    //imagenes
    productData["img"] = await page.$eval(
      "#productmainimageitem",
      (img) => img.src
    );
    //info
    productData["info"] = await page.$eval(
      ".productextrainfo",
      (info) => info.innerText
    );
    //descripción
    productData["description"] = await page.$eval(
      ...
      (description) => description.innerText.slice(0, 200) + "..."
    );
...
const scrap = async (url) => {
  try {
    ...
    /********** A RELLENAR page.$eval(selector, function) buscas en el dom la query que necesitas, funcion que quieres que haga con esos datos *********/
    const tmpurls = await page.$$eval(".productImage div div a", (res) =>
      res.map((a) => a.href)
    );
